...
<div class="row">
    ...
    <div class="column-responsive column-80">
        <div class="users view content">
            ...
            <div class="related">
                <h4><?= __('Related Rbac Users Roles') ?></h4>
                <?php if (!empty($user->rbac_users_roles)) : ?>
                <div class="table-responsive">
                    <table>
                        <tr>
                            <th><?= __('Id') ?></th>
                            <th><?= __('User Id') ?></th>
                            <th><?= __('Rbac Role Id') ?></th>
                            <th><?= __('Status') ?></th>
                            <th class="actions"><?= __('Actions') ?></th>
                        </tr>
                        <?php foreach ($user->rbac_users_roles as $rbacUsersRoles) : ?>
                        <tr>
                            <td><?= h($rbacUsersRoles->id) ?></td>
                            <td><?= h($rbacUsersRoles->user_id) ?></td>
                            <td><?= h($rbacUsersRoles->rbac_role_id) ?></td>
                            <td><?= h($rbacUsersRoles->status) ?></td>
                            <td class="actions">
                                <?= $this->Html->link(__('View'), ['controller' => 'RbacUsersRoles', 'action' => 'view', $rbacUsersRoles->id]) ?>
                                <?= $this->Html->link(__('Edit'), ['controller' => 'RbacUsersRoles', 'action' => 'edit', $rbacUsersRoles->id]) ?>
                                <?= $this->Form->postLink(__('Delete'), ['controller' => 'RbacUsersRoles', 'action' => 'delete', $rbacUsersRoles->id], ['confirm' => __('Are you sure you want to delete # {0}?', $rbacUsersRoles->id)]) ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </table>
